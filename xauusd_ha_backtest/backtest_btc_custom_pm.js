...
function runBTCCustomPM() {
    ...
    for (let i = 1; i < df.length; i++) {
        ...
        // ONLY 6:00 PM to 9:45 PM IST
        let is_blockout = false;
        if (ist_minute_total >= 1080 && ist_minute_total < 1305) {
            is_blockout = true;
        }
    ...
    console.log("BTC PM Blockout Only. Trades: " + trades.length + " | Win Rate: " + win_rate.toFixed(2) + "% | PnL: $" + total_pnl.toFixed(2));
}
